  ...
  template: `
    <div class="fixed bottom-0 left-0 w-full h-32 z-50 pointer-events-none flex items-end justify-center pb-8 group">
      ...
      <div class="w-full max-w-3xl px-4 pointer-events-auto transition-all duration-500 translate-y-32 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 focus-within:translate-y-0 focus-within:opacity-100">
        <div class="bg-[var(--color-eden-surface)] backdrop-blur-2xl border border-[var(--color-eden-border)] rounded-2xl p-3 shadow-[0_10px_50px_rgba(0,0,0,0.5)] flex items-center gap-4 transition-all duration-500 focus-within:border-white/50 focus-within:shadow-[0_0_25px_rgba(255,255,255,0.2)] hover:border-white/20">
          
          <mat-icon class="text-[var(--color-eden-neon)] animate-pulse ml-2">auto_awesome</mat-icon>
          <input 
            #intentInput
            type="text" 
            placeholder="Inject intent into EDEN (e.g. 'Create Auth Node' or '/local --version')..."
